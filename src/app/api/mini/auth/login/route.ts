...
import { createJwSessionToken, createMockSessionToken } from "@/lib/session/store";
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  if (isAdminCredential(studentId, password)) {
    const token = createMockSessionToken("admin");
    return NextResponse.json({ ok: true, token });
  }

  try {
    const result = await loginToJw(studentId, password);
    const token = createJwSessionToken(result.cookieHeader);
    return NextResponse.json({ ok: true, token });
  } catch (error) {
