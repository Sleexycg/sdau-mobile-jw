...
import { fetchScores, fetchStudentProfile } from "@/lib/jw/client";
...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  try {
    ...
    const scoreData = await fetchScores(sessionCookieHeader, term || undefined);
    ...
    return NextResponse.json({ ok: false, code: "JW_UNAVAILABLE", message: "成绩拉取失败，请稍后再试" }, { status: 503 });
  }
}