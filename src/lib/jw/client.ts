...
export async function fetchTimetable(cookieHeader: string, term: string): Promise<TimetableFetchResult> {
  ...
  const candidates: Array<{ path: string; method?: "GET" | "POST"; body?: URLSearchParams }> = [
    {
      path: "/xskb/xskb_list.do?viweType=0",
      method: "GET",
    },
    {
      path: "/xskb/xskb_list.do",
      method: "GET",
    },
    {
      path: `/xskb/xskb_list.do?viweType=0&xnxq01id=${encodeURIComponent(normalizedTerm)}`,
      method: "GET",
    },
  ];
  ...
  for (const candidate of candidates) {
    ...
    if (!looksLikeTimetablePage(response.text)) {
      continue;
    }
